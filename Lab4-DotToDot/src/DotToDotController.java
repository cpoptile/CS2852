...
public class DotToDotController {
    ...
    /**
     *  Method sets list to be used by the Picture class as a LinkedList
     *  that uses iterator to remove dots
     */
    @FXML
    public void linkedListIteratorStrategy(){
        dotList = new LinkedList<>();
        strategy = "LinkedListIterator";
    }
    ...
    public void updateNumberOfDots(){
        try {
            ...
            newList.clear();
            newPicture = new Picture(picture, newList);
            GraphicsContext gc = canvas.getGraphicsContext2D();
            gc.clearRect(0, 0, canvas.getWidth(), canvas.getHeight());
            newPicture.removeDots(Integer.parseInt(numberOfDots.getText()), strategy);
            newPicture.drawLines(canvas);
            newPicture.drawDots(canvas);
            updateDotNumber(newPicture);
            time.setText(String.valueOf(newPicture.getTime()));
        } catch (IllegalArgumentException e){
